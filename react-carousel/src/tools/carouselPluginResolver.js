...
const carouselPluginResolver = (plugins, carouselProps, trackRef, trackContainerRef, nodeRef) => {
  const carouselPlugins = carouselProps?.plugins.map(plugin => {
    if (typeof(plugin) === 'string') {
      return pluginsFunc[plugin.toUpperCase()] && pluginsFunc[plugin.toUpperCase()]({
        pluginProps: {
          ...carouselProps,
          children: carouselProps.children ? carouselProps.children : carouselProps.slides,
        },
  ...
  const customProps = carouselPlugins
    .map(plugin => plugin.carouselProps && plugin.carouselProps());

  const beforeCarouselItems = carouselPlugins
    .map(plugin => plugin.beforeCarouselItems && plugin.beforeCarouselItems()) || [];

  const afterCarouselItems = carouselPlugins
    .map(plugin => plugin.afterCarouselItems && plugin.afterCarouselItems()) || [];
